import * as allIconDefs from '@hankliu/icons-svg';
import { IconDefinition } from '@hankliu/icons-svg/es/types';
...
const svgPkg = require.resolve('@hankliu/icons-svg');
// const svgPkgDir = packageDirectorySync({cwd: path.dirname(svgPkg)});
...
function svg2base64(svgPath: string, size = 50) {
  ...
  const svgWithStyle = svg
    ...
    // https://github.com/hankliu/hankliu-icons/blob/a02cbf8/packages/icons-svg/templates/helpers.ts#L3-L6
...
import <%= svgIdentifier %>Svg from '@hankliu/icons-svg/lib/asn/<%= svgIdentifier %>';
import HankLiuIcon, { HankLiuIconProps } from '../components/HankLiuIcon';

const <%= svgIdentifier %> = (
  props: HankLiuIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <HankLiuIcon {...props} ref={ref} icon={<%= svgIdentifier %>Svg} />;

<% if (svgBase64) { %> /**![<%= name %>](<%= svgBase64 %>) */ <% } %>
const RefIcon: React.ForwardRefExoticComponent<
  Omit<HankLiuIconProps, 'ref'> & React.RefAttributes<HTMLSpanElement>
> = React.forwardRef<HTMLSpanElement, HankLiuIconProps>(<%= svgIdentifier %>);
...
// 将 lib/icons 下的所有icons，拷贝到全局路径下
