import { generate as generateColor } from '@hankliu/colors';
import type { AbstractNode, IconDefinition } from '@hankliu/icons-svg/lib/types';
...
export function warning(valid: boolean, message: string) {
  warn(valid, `[@hankliu/icons] ${message}`);
}
...
export const useInsertStyles = (eleRef: React.RefObject<HTMLElement>) => {
  ...
  if (prefixCls) {
    mergedStyleStr = mergedStyleStr.replace(/hlui-icon/g, prefixCls);
  }
  ...
  useEffect(() => {
    ...
    updateCSS(mergedStyleStr, '@hankliu-icons', {
      prepend: true,
      csp,
      attachTo: shadowRoot,
    });
